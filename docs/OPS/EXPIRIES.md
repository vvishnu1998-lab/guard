...
| E15 | **SendGrid card failing** | SendGrid billing | **UNVERIFIED — failing now** | Vishnu | Payment method declining. SendGrid sends `dailyShiftEmail`, `missedShiftAlert` and the handoff FYI, so a suspension silently stops every admin/client email — none of which the triage loop currently checks. Fix the card as the immediate action; migration away is **N22**. |
