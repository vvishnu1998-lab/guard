...
| E14 | **[VISHNU] AWS free plan ends 2026-09-30** | AWS account | **2026-09-30** (hard) | Vishnu | **Bucket `guard-media-prod` (us-east-1) is at risk.** Upgrade to a standard paid plan **before** the date. This bucket holds every report photo, ping photo and clock-out photo; losing or suspending it is customer-visible and unrecoverable. Note S3 versioning is ON and lifecycle rule `noncurrent-30d` expires noncurrent versions after **60** days (`NoncurrentDays 60` — the id says 30; read 2026-09-26, OPEN-ITEMS N132), so stored volume runs up to ~60 days of overwritten and deleted bytes above the live object count. **Status 2026-09-26: OPEN.** Vishnu reported the upgrade under way the same day; not confirmed when this was recorded, and not readable from here (IAM user `starguard-app` is denied `freetier:GetAccountPlanState`). The deadline stands until Vishnu records the upgrade. Guard photo uploads write to the bucket continuously; the monthly report job next writes to it 2026-10-01 12:00 UTC. **This is the nearest dated row in this file — treat it as the deadline.** |
