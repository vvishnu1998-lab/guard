...
|---|---|---|---|---|---|
| E1 | Apple Developer Program membership | developer.apple.com | **2027-05-05** | Vishnu | Lapse blocks all iOS builds, TestFlight, and App Store updates. Annual. **Date is DERIVED, not read: 2026-05-05 payment + 1 year, per Vishnu 2026-09-06. Verify in the developer portal and replace this note with the portal's own renewal date.** The arithmetic is the weak part — Apple renews on the anniversary of the *membership*, which is not always the payment date, and an auto-renew that fails silently lapses on the same day either way. **This is now the second-nearest dated expiry after E14 (AWS, 2026-09-30), and the first that would block a store release.** |
...
| E14 | **AWS account plan — DONE 2026-09-26** | AWS account | — | Vishnu | **DONE 2026-09-26: the account plan is PAID, status ACTIVE, credits 119.94 USD — verified ~13:58 PT by Vishnu as the account owner in CloudShell (`aws freetier get-account-plan-state`). The free-plan expiry (2026-09-30) does not apply.** No date in the date column, as with E15: the free-plan end date no longer applies, and `scripts/ops/triage.sh` `c_ahead` reads that column and would keep reporting the row as expiring. Not re-readable with the app's credentials (IAM user `starguard-app` is denied `freetier:GetAccountPlanState`). Whether the credits carry an expiry was not recorded. History kept: this row read "[VISHNU] AWS free plan ends 2026-09-30", date 2026-09-30 (hard), because bucket `guard-media-prod` (us-east-1) — every report, ping and clock-out photo — was at risk if the plan lapsed; it was recorded OPEN earlier on 2026-09-26 while the upgrade was under way. Note S3 versioning is ON and lifecycle rule `noncurrent-30d` expires noncurrent versions after **60** days (`NoncurrentDays 60` — the id says 30; read 2026-09-26, OPEN-ITEMS N132), so stored volume runs up to ~60 days of overwritten and deleted bytes above the live object count. |
| E15 | **SendGrid card — RESOLVED 2026-09-06** | SendGrid billing | — | Vishnu | **RESOLVED 2026-09-06 (card fixed, per Vishnu). No date in the date column: a payment method has no expiry we can read.** History kept for the incident cross-references. Payment method was declining. **It caused a real outage**: SendGrid returned 401 on every send for **6 d 18 h** (last good `2026-08-25 23:10:01Z`, next good `2026-09-01 17:00:01Z`), which cost ~7 STARNET daily client reports and — via a 5-minute retry loop emitting one Sentry event per recipient — exhausted the monthly Sentry quota and blinded error monitoring for 94 h. See `INCIDENTS/2026-09-01-unauthorized-burst.md` (N28) and `INCIDENTS/2026-09-06-sentry-rate-limited.md` (N27). **Banner confirmed gone 2026-09-06 (per Vishnu)** — so the account is cleared, not merely re-carded, and residual (1) is closed. Residual, tracked under N22 not here: (2) **The 2026-09-01 17:00Z recovery is still unexplained**: the card was fixed on 09-06, five days later, so something else restored service that day (a retried charge, or a grace period) and whatever it was could lapse again. The loop now watches this directly — `hours_since_last_successful_email` in the `failures-24h` collector, P1 above 26 h. Migration away is **N22**, now recommended rather than optional. |
