...
## Prerequisite — the shell variable

`DATABASE_PUBLIC_URL` must be set from `~/guard/.env`. The internal hostname
(`postgres.railway.internal`) resolves only inside Railway and will hang from a
workstation.

```bash
cd ~/guard
export DATABASE_PUBLIC_URL="$(grep -E '^DATABASE_PUBLIC_URL=' .env | cut -d= -f2-)"
printf 'host set: %s\n' "$(printf '%s' "$DATABASE_PUBLIC_URL" | sed -E 's#.*@##')"
```

The last line prints only the host and port, never the password. If it prints
nothing, the variable is not in `.env` and the rest of this runbook will not
work.
...
```bash
psql "$DATABASE_PUBLIC_URL" -1 -v ON_ERROR_STOP=1 -f src/db/schema_v67.sql
```
...
```bash
psql "$DATABASE_PUBLIC_URL" -c "SELECT to_regclass('public.cron_heartbeats');"
```
...
```bash
psql "$DATABASE_PUBLIC_URL" -v ON_ERROR_STOP=1 -f ~/guard/scripts/ops/readonly-column-revoke.sql
```
...
```bash
psql "$DATABASE_PUBLIC_URL" -f ~/guard/scripts/ops/readonly-column-revoke.verify.sql
```
...
```bash
psql "$DATABASE_PUBLIC_URL" -c "SELECT COUNT(*) AS starnet_open FROM shift_sessions ss JOIN guards g ON g.id = ss.guard_id WHERE ss.clocked_out_at IS NULL AND g.company_id = '27c4d404-8769-49ca-bfd6-93cb9b890067';"
```
...
```bash
psql "$DATABASE_PUBLIC_URL" -c "SELECT c.name, COUNT(*) FROM shift_sessions ss JOIN guards g ON g.id = ss.guard_id JOIN companies c ON c.id = g.company_id WHERE ss.clocked_out_at IS NULL GROUP BY c.name;"
```
...
```bash
psql "$DATABASE_PUBLIC_URL" -c "SELECT job_name, last_result, NOW()-last_tick_at AS age FROM cron_heartbeats ORDER BY age;"
```
...
```bash
psql "$DATABASE_PUBLIC_URL" -c "GRANT SELECT ON guards, company_admins, clients, guard_devices, password_reset_tokens, revoked_tokens, login_attempts, vishnu_state TO claude_readonly;"
```
