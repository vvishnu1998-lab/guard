  ...
  async function handleForgotPassword() {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: resetEmail, portal: 'client' }),
      });
      const data = await res.json();
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white/[0.04] border border-white/[0.1] rounded-lg px-4 py-3.5 text-white text-sm placeholder-white/20 focus:border-blue-400/60 focus:ring-1 focus:ring-blue-400/40 focus:bg-white/[0.06] focus:outline-none transition-all"
