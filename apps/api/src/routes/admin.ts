...
router.post('/companies/:id/admins', requireAuth('vishnu'), async (req, res) => {
  ...
  // Auto-generate a 12-char temp password when the caller omits one.
  // Response echoes it back only in the auto-gen branch (mirrors clients.ts).
  const autoGenerated = !password;
  const tempPassword  = autoGenerated ? generateTempPassword(12) : String(password);
  if (!autoGenerated) {
  ...
  const result = await pool.query(
    `INSERT INTO company_admins (id, company_id, name, email, password_hash, is_primary, is_active, must_change_password)
  ...
  // Fire-and-forget welcome email. is_primary decides the template (bootstrap
  // "welcome, contact support" vs "created by X, contact your primary admin").
  const welcomeFn = is_primary ? sendPrimaryAdminWelcomeEmail : sendSecondaryAdminWelcomeEmail;
  welcomeFn({
    admin_id:         newAdmin.id,
    admin_name:       newAdmin.name,
    admin_email:      newAdmin.email,
    company_id:       req.params.id,
    // Only consumed by the secondary template. For a primary admin created
    // by vishnu there is no creator human — sub is the vishnu sentinel UUID.
...
router.post('/company-admins', requireAuth('company_admin'), async (req, res) => {
  ...
  // Auto-generate a 12-char temp password when the caller omits one.
  // Response echoes it back only in the auto-gen branch (mirrors clients.ts
  // and the vishnu-bootstrap admin path).
  const autoGenerated = !password;
  const tempPassword  = autoGenerated ? generateTempPassword(12) : String(password);
  if (!autoGenerated) {
    const policyErr = validatePassword(tempPassword);
  ...
  sendSecondaryAdminWelcomeEmail({
    admin_id:         newAdmin.id,
    admin_name:       newAdmin.name,
    admin_email:      newAdmin.email,
