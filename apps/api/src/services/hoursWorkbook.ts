...
export function buildHoursWorkbook(data: HoursExportDataset): ExcelJS.Workbook {
  ...
  note('', '');
  note('Scheduled', 'The shift’s scheduled window. On a DETAIL row this is the whole shift. In aggregates a handoff shift is split across its sessions in proportion to payable hours (equally when the shift has no payable time), so aggregate scheduled totals do not equal the sum of the detail column.');
  note('Actual', 'Clock-out minus clock-in, raw, no truncation. Includes time before the scheduled start and after the scheduled end. Used for the OVER and OFFPOST_ANOMALY flags; totals, Variance and Coverage use Payable.');
  note('Payable', 'Clocked-in time inside the scheduled window: from the later of clock-in and scheduled start, to the earlier of clock-out and scheduled end, never below zero. Breaks are not subtracted. This is the figure the totals, Variance, Coverage % and SHORT are computed from. It is not the legacy stored Total Hours, which clipped the start only.');
