...
- `shifts`: no `tenant_id`, no `timezone`, no `updated_at`. Scope via `guard_id`+`site_id`. NOT NULL: `site_id`, `scheduled_start`, `scheduled_end`, `status`. **`guard_id` IS NULLABLE** — it is NULL for every `status='unassigned'` shift (**9 of 522 rows, verified 2026-09-10**; this line previously said 15 of 189). This file once listed `guard_id` as NOT NULL, which is wrong and would break any code that assumes a shift always has a guard. `status='unassigned'` <-> `guard_id IS NULL` still holds in both directions across all 522 rows (0 violations either way) but is **NOT enforced by any constraint** — check both, never infer one from the other. `shifts_status_check` admits six values since v75: `unassigned, scheduled, active, completed, missed, cancelled`.
...
- **`POST /shifts/:id/swap-response` and `/handoff-response` DO emit machine codes — branch on the code, not on `.status`.** *(Corrected 2026-09-10. This file previously said these two routes emit prose and that `.status` was the only stable signal. N46 made that false at `8b799d7`; the old text would have had a client ignore codes that exist.)* Each route emits FOUR 409s — not one — and only one of the eight is "already responded to, or it expired": `SWAP_NOT_PENDING`, `SHIFT_NOT_SCHEDULED`, `SWAP_STALE_REASSIGNED`, `RECIPIENT_OVERLAP`, `HANDOFF_NOT_PENDING`, `SHIFT_NOT_ACTIVE`, `HANDOFF_STALE_REASSIGNED`, `OPEN_SESSION_EXISTS`. All carry the hybrid shape `{ code, error, message, …extra }` with the enum in BOTH `code` and `error`, so mobile's `err.code` (derived from `error`, `lib/errors.ts:72`) and `err.details.code` both match. 422 (swap only) still passes through verbatim — the server's sentence is the only place the eligibility detail exists. Mobile reads this in `lib/respondErrorCopy.ts`, proven by `apps/mobile/scripts/check-respond-copy.ts`.
- **The rule is not "enum in both fields" — it is PUT THE ENUM WHERE THE CONSUMER CAN READ IT, and that depends on which client calls the route.** Mobile derives `ApiError.code` from `body.error` and has no other path to it, so a mobile-called route must put the enum there. **Web's `ApiError` (`apps/web/lib/adminApi.ts`) has NO `code` field at all** — it keeps the parsed body on `.body`, so a web caller reads `err.body.code`, and `adminApi.ts:73` puts `body.error` straight into `ApiError.message`, which admin screens render verbatim. So on a WEB-ONLY route the enum must go in `code` only and `error` must keep its prose, or an admin reads a raw enum where a sentence used to be. `PATCH /shifts/:id/cancel` is shaped that way deliberately and says so in its docblock.
...
## Shipped state — snapshot 2026-09-10

**A snapshot, not a standing fact.** Re-verify before asserting any of it: `git rev-parse origin/main`, the update CLI's production channel view, and the tail of `migrate.ts`. This block has gone stale twice — the 2026-08-25 version survived until 2026-09-10 still claiming schema v59 when prod was on v75, sixteen versions behind.

| thing | value |
|---|---|
| `origin/main` | `9eec15a` — N55 month-name dates |
| production OTA group | `9386a6d2-ce69-493c-8273-c4a9a45c957b` (**runtime 1.0.17**, android `01a08a24-c326-7a37-ab44-b7a831f40a07`, ios `01a08a24-c326-7136-a71b-c143de6575f6`) — N46 mobile half |
| prod schema | **v75** applied. **v76 is free** — but read `migrate.ts`, never this line |
| mobile batch branch | **UNVERIFIED** — local remote-tracking refs only reach `batch/mobile-9` without a fetch. Run `git ls-remote --heads origin 'batch/mobile-*'`. |

- **The migration chain replays clean end-to-end as of N43.** It had been unrunnable since 2026-08-29 (`schema_v5.sql` aborting at file 6 with SQLSTATE 23514); v5 is now guarded on DATA, and `schema_v75` brings a clean build in line with prod. Proven on a local PG 18.6: 76/76 files, zero schema divergence against prod across tables, columns, NOT NULLs, CHECKs, keys and indexes, and idempotent on re-run. **`npm run db:seed` was removed** — `src/db/seed.ts` never existed in this repo's history.
